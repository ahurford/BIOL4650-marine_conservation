...
Let's explore the _protected areas_ data!
...
7. To extract a column of the data, see [Handling the data](https://ahurford.github.io/quant-guide-all-courses/handling-data.html). You may use either Approach 1: base R or Approach 2: `dplyr` (generally, the `dplyr` commands tend to be simpler, however, it is good to also know the base R commands).
...
8.  We want to separate protected areas identified as exclusively terrestrial from those that include marine areas. Try this in the `Console`:

```{r, results=F}
filter(data1, MARINE == 0)
```

9. To filter data that include marine areas, we run this code in the `Console`:

```{r, results=F}
data2 <- filter(data1, MARINE != 0)
```

10. We want to compute the average size of coastal MPAs. Before we doing this, we will need to filter the data to remove rows with null values in the `REP_AREA` column. Try this code in the `Console`
...
The median size of the coastal MPAs can lso be found by running a similar code as follows:
```{r, results=F}
...
1. First, we would like to graph the the number of areas in each level of protection. To do this, we will have to count the number of areas in each level of protection by running this code in the `Console` 
```{r, results=F}
data2 <- data1 %>% group_by(IUCN_CAT) %>% summarise(n())
```

We could also attain the same results by using the `group_by` and `tally` functions. I will demonstrate this in the next line

```{r, results=F}
data2 <- group_by(df1, IUCN_CAT)
tally(data2)
```
...
```{r, results=F}
barplot(data2$`n()`, ylab="number(No. of areas)",
        names.arg=c("Ia","Ib","II","III","IV","V","VI"), las=2, main="Number of areas for each level of protection")
```

2. Now we would like to calculate the amount of area that is strongly protected. We can do this by grouping by the `IUCN_CAT` column of the dataset and summing the `REP_AREA` accordingly. Let's run this code in the `Console`
```{r, results=F}
data3 <- data1 %>% group_by(IUCN_CAT) %>% summarise(REP_AREA = sum(REP_AREA))
```

The barplot showing the area allocations could be plotted by running this code in the `Console`

```{r, results=F}
barplot(data3$REP_AREA, ylab="Area(Amt. land and water/km^2)",
        names.arg=c("Ia","Ib","II","III","IV", "NA", "NA","NR" ,"V", "VI"), las=2, main="Area of land and water for each level of protection")
``` 
