...
```
## Naming a subsetted dataframe {-}
This section is focused on subsetting the entire dataset which was named as `data1` and saving it with a new name called `data2`.

1. We want to filter the data to include marine areas and exclude terrestrial areas and save it as `data2`. Let's run this code in the `Console`:

```{r, results=F}
data2 <- filter(data1, MARINE != 0)
```

2. We want to compute the average size of coastal MPAs. Before we doing this, we will need to filter the data to remove rows with null values in the `REP_AREA` column. Try this code in the `Console`
...
```{r, results=F}
barplot(data3$n, ylab="Number of areas",
        names.arg=c("Ia","Ib","II","III","IV","V","VI"), las=1, main="Number of areas for each level of protection")
```
...
```{r, results=F}
barplot(data4$REP_AREA, ylab="Area(Amt. land and water/km^2)",
