...
** To be corrected, numbering and screenshots**
...
Open R Studio and save a new R Script. In the top menu select `File > New File > R Script`, then `File > Save As`. Save the `.R` (the R Script) to the folder you made on your D: drive (or Desktop): 
...
3. First we will [load your data](https://ahurford.github.io/quant-guide-all-courses/data-entry.html#loading-or-importing-data):
...
Again, with is a double negative: we ask if the row is `NA`, and if the answer is `FALSE` (not an `NA`) we keep the row in `data3`.
...
The line of code that makes `data3` is an essential step in the analysis so you will want to add it as the next line of code in your R Script:
...
We would like to calculate the total area that is 'no take'. Let's check the appropriate column of the data:

```{r}
data3$NO_TK_AREA
```
In the Ecuador example data this value is always 0 (therefore it is not a good example). However, we can still lean the syntax to take the sum:
...
Note that for the original Ecuador data this did not work because all entries in the `IUCN_CAT` column were `Not Reported` or `Not Applicable` (`ecuador.csv` is a supplemented version of the true Ecuador data with some `IUCN_CAT` set to `II` and `III`). In this example, row 3 is `Not Reported`. We can remove this row:
...
In this section we will made graphs of `data3, `data4` and `data5`.
...
The next lines of your R Script would look like this:
```{r,out.width="80%",echo=F}
